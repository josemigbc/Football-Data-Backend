...
def get_competition(league_name_url: str) -> dict:
    ...
    standings = get_standings(
        league_name_url, season=competition.get("season"))
...
def manage_teams(soup: Tag | NavigableString | None) -> list[dict] | list[None]:
    ...
    for team in teams_row_soup:
        ...
        if logo_soup:
            ...
            id = logo.split(
                "https://secure.espn.com/combiner/i?img=/i/teamlogos/soccer/500/")[1].split(".")[0]
            props = [prop.text.strip() for prop in team.find_all('td')]
            team_data = dict(zip(fields, props))
            national_league_name = team.find(class_="league")['data-slug']
            try:
                national_league = FootballCompetition.objects.get(
                    name_url=national_league_name).id
            except:
